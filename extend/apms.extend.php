...
// APMS Version
$yc5_ver = (defined('G5_USE_SHOP')) ? ' / 영카트 5.3.3.3.1(패치 YC5.5.8.2.8)' : '';
define('APMS_VERSION', '아미나빌더 1.8.54 / 그누보드 5.3.3.3(패치 G5.5.8.2.8)'.$yc5_ver);
...
define('APMS_YOUTUBE_KEY', $xp['youtube_key']);
...
// SPAM Chkeck
function apms_spam_check($content) {
	global $xp;

	$content = trim($content);
	if(!$content) return;
	
	$spam_msg = trim($xp['spam_msg']);
	if(!$spam_msg) return;

	$spam = explode(",", $spam_msg);
	$cnt = count($spam);
	for ($i=0; $i < $cnt; $i++) {
		$str = trim($spam[$i]);
		if(!$str) continue;
		...
		if ($pos !== false) {
			alert('접수되었습니다.', G5_BBS_URL.'/spam.php');
		}
	}
...
// 글, 댓글 스팸체크
if(!$is_admin) { //관리자가 아닐 경우만 체크
	if(isset($_POST['wr_subject']) || isset($_POST['wr_content'])) {
		apms_spam_check($_POST['wr_subject'].''.$_POST['wr_content']);
	}
}
