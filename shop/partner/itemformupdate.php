...
echo '<pre>';
include_once(G5_PATH.'/lib/item.extends.lib.php');
...
if(!$is_auth && $w != "u") {
	...
	$upoint_where = "";
	if($ca_id) {
		$upoint_where .= " ca_id = '{$ca_id}' ";
	}
	if($upoint_where && $ca_id2) {
		$upoint_where .= " or ";
	}
	if($ca_id2) {
		$upoint_where .= " ca_id = '{$ca_id2}' ";
	}
	if($upoint_where && $ca_id3) {
		$upoint_where .= " or ";
	}
	if($ca_id3) {
		$upoint_where .= " ca_id = '{$ca_id3}' ";
	}
	if($upoint_where) {
		$upoint_where = "and (".$upoint_where.")";
	}
	...
	if($uplimit) { // 제한값이 있으면...
		$row = sql_fetch(" select count(*) as cnt from {$g5['g5_shop_item_table']} where pt_id = '{$member['mb_id']}' and date_format(it_time, '%Y-%m-%d') = '".G5_TIME_YMD."' and (ca_id = '{$uplimit_ca}' or ca_id2 = '{$uplimit_ca}' or ca_id3 = '{$uplimit_ca}') ");
		if($row['cnt'] && $row['cnt'] >= $uplimit) {
		    alert("선택하신 분류 중 ".$uplimit_name." 분류는 하루 최대 ".$uplimit."개까지만 등록이 가능합니다.\\n\\n등록갯수 초과로 오늘은 더이상 등록할 수 없습니다.");
		}
	}

	// 등록비 체크	-가장 큰 등록비를 가져옴
	$row1 = sql_fetch(" select ca_id, pt_point from {$g5['g5_shop_category_table']} where pt_point > 0 $upoint_where order by pt_point desc ");

	$upoint = (int)$pt['pt_point'] + (int)$row1['pt_point'];

	if($upoint) {
		$new_upoint = $upoint * (-1); //부호 전환
		if($w == "u") {
			//기존 내역
			$row2 = sql_fetch(" select po_point from {$g5['point_table']} where mb_id = '{$member['mb_id']}' and po_rel_table = '@regit' and po_rel_id = '{$member['mb_id']}' and po_rel_action = '$it_id' order by po_id desc ");
...
/***
 * 상훔수정에 item.extends 관련 필드 값 수정 작
 *
 */
...
setItemVersionConfig(array(
    ...
    'item_target' => 'A',
    'item_id' => $it_id,
    'item_version' => $_POST['item_version']??'1.0.0',
...
//     echo '<br> delete '.$key.'  => '.$_POST['item_ext_link_delete'][$key].'      <br>';
// }
...
foreach ($_POST['item_ext_link_key'] as $keyIndex => $linkKey){
    ...
    if($isDelete){
        ...
        $rsSetting = setItemLinkExtends(array(
            ...
            'item_id' => $it_id,
            'item_ext_link_key' => $linkKey,
            'item_ext_link_name' => $_POST['item_ext_link_name'][$linkKey],
            'item_ext_link' => $_POST['item_ext_link'][$linkKey],
            'item_ext_link_download_code' => $_POST['item_ext_link_download_code'][$linkKey]??'',
            'item_ext_link_is_buy' => !empty($_POST['item_ext_link_is_buy'][$linkKey]) && $_POST['item_ext_link_is_buy'][$linkKey] == 'on'?'Y':'N',
