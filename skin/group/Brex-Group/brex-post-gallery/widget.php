...
if(!G5_IS_MOBILE) {
	...
	add_stylesheet('<link rel="stylesheet" href="'.$widget_url.'/widget-m.css">', 0);
}
...
<div id="<?php echo $widget_id;?>" class="brex-post-gallery">
	<?php 
		if(!G5_IS_MOBILE) {
			...
			if($wset['cache'] > 0) { // 캐시적용시
				echo apms_widget_cache($widget_path.'/widget.rows-m.php', $wname, $wid, $wset);
			} else {
				include($widget_path.'/widget.rows-m.php');
			}
		}
	?>
