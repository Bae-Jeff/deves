...
$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_ini\');">기본사항</a></li>';
$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_img\');">이미지첨부</a></li>';
$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_attach\');">파일첨부</a></li>';
$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_relation\');">관련자료</a></li>';
$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_opt\');">옵션사항</a></li>';
$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_explan\');">추가내용</a></li>';
$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_sell\');">판매설정</a></li>';
if($is_auth) { // 관리자일 때
	$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_event\');">관련이벤트</a></li>';
	$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_optional\');">상하단내용</a></li>';
	$pg_anchor .='<li><a href="javascript:apms_show(\'anc_sitfrm_extra\');">여분필드</a></li>';
}
$pg_anchor .='</ul>';
...
<section id="anc_sitfrm_ini" class="anc-section">
    <h2 class="h2_frm">기본사항</h2>
	<div class="tbl_frm01 tbl_wrap">
        <table>
        <caption>기본사항 입력</caption>
        <colgroup>
            <col class="grid_3">
            <col>
        </colgroup>
        <tbody>
        <tr>
            <th scope="row"><label for="it_name">서비스종류</label></th>
            <td>
				<?php echo help("서비스 종류에 따라 주문 및 결제후 이용방법이 달라집니다."); ?>
				<select name="pt_it" id="pt_it" required>
					<option value="">선택해 주세요.</option>
					<?php echo apms_pt_it($it['pt_it']); ?>
				</select>
                <a href="<?php echo G5_SHOP_URL; ?>/item.php?it_id=<?php echo $it_id; ?>" target="blank" class="btn_frmline">글확인</a>
		...
		<tr>
            <th scope="row"><label for="it_name">글제목</label></th>
            <td>
		...
		<?php if($is_auth || $default['pt_comment_use']) { ?>
			<tr>
				<th scope="row"><label for="pt_comment_use">댓글</label></th>
				<td>
		...
		<?php } ?>
		<tr>
            <th scope="row"><label for="pt_tag">태그</label></th>
            <td>
                <?php echo help("등록할 태그를 콤마(,)로 구분해서 입력합니다."); ?>
                <input type="text" name="pt_tag" value="<?php echo get_text($it['pt_tag']); ?>" id="pt_tag" class="frm_input sl">
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="pt_link1">링크1</label></th>
            <td>
        ...
        <tr>
            <th scope="row"><label for="pt_link2">링크2</label></th>
            <td>
		...
		<tr>
            <th scope="row"><label for="it_use">출력</label></th>
            <td>
                <?php echo help("체크하셔야 발행되며, 잠시 노출을 중단하거나 재고가 없을 경우에 체크를 해제해 놓으면 출력되지 않습니다."); ?>
                <label><input type="checkbox" name="it_use" value="1" id="it_use" <?php echo ($it['it_use']) ? "checked" : ""; ?>> 예</label>
            </td>
        </tr>
		<tr>
            <th scope="row"><label for="it_use">주문</label></th>
            <td>
                <?php echo help("체크하시면 판매설정을 하셔도 주문기능이 작동하지 않습니다."); ?>
                <label><input type="checkbox" name="pt_order" value="1" id="pt_order" <?php echo ($it['pt_order']) ? "checked" : ""; ?>> 주문기능을 사용하지 않습니다.</label>
            </td>
        </tr>
		<?php if($is_auth || $default['pt_review_use']) { ?>
			<tr>
				<th scope="row"><label for="pt_review_use">후기</label></th>
				<td>
	                <?php echo help("체크하시면 쇼핑몰 설정과 관계없이 후기 등록이 가능합니다."); ?>
...
<section id="anc_sitfrm_opt" class="anc-section">
	...
	<div class="tbl_frm01 tbl_wrap">
		...
		<tr>
            ...
            <td>
                <?php if ($w == '') { // 추가 ?>
                    ...
                    <?php echo ($is_auth) ? help("글코드는 10자리 숫자로 자동생성합니다. 직접 글코드를 입력시 영문자, 숫자, - 만 입력 가능합니다.") : help("글코드는 10자리 숫자로 자동생성합니다."); ?>
        ...
        <tr>
            <th scope="row"><label for="it_basic">글요약</label></th>
            <td>
                <?php echo help("글에 대한 추가적인 설명이 필요한 경우에 입력합니다. 목록, 메인 등에 노출되며 HTML 입력도 가능합니다."); ?>
                <input type="text" name="it_basic" value="<?php echo get_text($it['it_basic']); ?>" id="it_basic" class="frm_input sl">
            </td>
        </tr>
		...
		<?php if($is_auth || !$is_reserve_none) { ?>
			<?php if($is_auth || $is_reserve) { // 관리자 일 때 ?>
				<tr>
					<th scope="row"><label for="pt_reserve">발행예약</label></th>
					<td>
						<?php if(!$is_auth && $is_reserve_none) { ?>
							<?php if($it['pt_no']) { ?>
								<?php echo date("Y년 m월 d일 H시 i분", $it['pt_no']);?>에 발행되었습니다.
							<?php } else if($it['pt_reserve']) { ?>
								현재 <?php echo date("Y년 m월 d일 H시 i분", $it['pt_reserve']);?>으로 발행예약되어 있습니다.
							<?php } else { ?>
								글등록 후 <?php echo $default['pt_reserve_none'];?>시간이 지나 예약설정 또는 변경이 불가능합니다.
							<?php } ?>
						...
						<?php } else { ?>
							<?php 
								if($is_auth) {
									if($is_reserve) {
										echo help("현재 ".$default['pt_reserve_day']."일 이내 등록글을 대상으로 ".$default['pt_reserve_cache']."분 간격으로 예약글을 체크하고 있습니다."); 
									} else {
										echo help("발행예약은 파트너관리 > 기본설정에서 예약가능일, 예약체크, 예약대상을 모두 설정해 주셔야 작동합니다."); 
									}
								} else {
			...
			<?php } ?>
			<tr>
				<th scope="row"><label for="pt_end">발행종료</label></th>
				<td>
					<?php if($is_reserve_none) echo help("발행종료는 파트너관리 > 기본설정에서 예약체크가 설정되어 있어야 작동합니다."); ?>
...
<section id="anc_sitfrm_explan" class="anc-section">
    <h2 class="h2_frm">추가내용</h2>

	<div class="tbl_frm01 tbl_wrap">
        <table>
        <caption>추가 글내용 입력</caption>
        <colgroup>
            <col class="grid_3">
            <col>
        </colgroup>
        <tbody>
		<tr>
            <th scope="row">추가 글내용</th>
            <td class="helper"> 
				구매한 회원에게만 출력되는 추가 글내용입니다.
			</td>
        </tr>
		<tr>
		...
		<tr>
            <th scope="row">모바일 추가 글내용</th>
            <td class="helper">
				구매한 회원에게만 출력되는 모바일 추가 글내용입니다.
			</td>
		</tr>
		<tr>
            <td colspan="2" class="iframe">
...
<div id="anc_sitfrm_sell" class="anc-section">
	...
	<section id="anc_sitfrm_compact">
		...
		<div id="sit_compact">
			...
			<select id="it_info_gubun" name="it_info_gubun">
				...
				<?php 
					$it_gubun = ($it['it_info_gubun']) ? $it['it_info_gubun'] : 'digital_contents';  // 초기값 
					echo apms_it_gubun($it['it_info_gubun'], $it_gubun);
				?>
			</select>
		</div>
	...
	<section id="anc_sitfrm_cost">
		...
		<div class="tbl_frm01 tbl_wrap">
			...
			<tr>
				<th scope="row"><label for="it_price">이용기간</label></th>
				<td>
					<?php echo help("다운로드, 감상 등의 구매 컨텐츠에 대한 구매이력 유지기간으로 설정일이 지나면 재구매해야 합니다."); ?>
					<input type="text" name="pt_day" value="<?php echo $it['pt_day']; ?>" id="it_price" class="frm_input" size="8"> 일까지 구매 후 이용가능
				</td>
			</tr>
			<?php if($is_auth) { // 관리자일 때만 출력 ?>
				<tr>
...
<section id="anc_sitfrm_img" class="anc-section">
    <h2 class="h2_frm">이미지첨부</h2>

	<div class="local_desc02 local_desc">
		{이미지:1}, {이미지:2} 와 같이 첨부이미지 번호를 입력하면 해당 첨부이미지를 내용에 출력할 수 있습니다.
	</div>
	...
	<div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <tr>
            <th scope="row"><label for="pt_img">출력여부</label></th>
            <td>
				<label><input type="checkbox" id="pt_img" name="pt_img" value="1"<?php echo ($it['pt_img']) ? ' checked' : '';?>> 썸네일보기 사용안함(스킨에 따라 작동하지 않을 수 있슴)</label>
            </td>
        </tr>
        <?php for($i=1; $i<=10; $i++) { ?>
        <tr>
...
<section id="anc_sitfrm_relation" class="srel anc-section">
    <h2 class="h2_frm">관련자료</h2>

    <div class="local_desc02 local_desc">
        <p>
            등록된 전체자료 목록에서 분류를 선택하면 해당 자료 리스트가 연이어 나타납니다.<br>
            자료 리스트에서 관련자료로 추가하시면 선택된 관련자료 목록에 <strong>함께</strong> 추가됩니다.<br>
            예를 들어, A 자료에 B 자료를 관련자료로 등록하면 B 자료에도 A 자료가 관련자료로 자동 추가되며, <strong>확인 버튼을 누르셔야 정상 반영됩니다.</strong>
        </p>
    </div>
        ...
        <section class="compare_left">
            ...
            <span class="srel_pad">
                <select id="sch_relation">
                    <option value=''>분류별 자료</option>
					<?php echo apms_it_category($is_auth, $is_partner);?>
                </select>
                <label for="sch_name" class="sound_only">제목 또는 상품명</label>
            ...
            <div id="relation" class="srel_list">
                <p>분류를 선택하거나 제목 또는 상품명 입력 후 검색해 주세요.</p>
            </div>
            <script>
            $(function() {
                $("#btn_search_item").click(function() {
                    ...
                    if(ca_id == "" && it_name == "") {
                        $relation.html("<p>분류를 선택하거나 제목 또는 상품명 입력 후 검색해 주세요.</p>");
                        return false;
                    }
                ...
                $(document).on("click", "#reg_relation .del_item", function() {
                    ...
                    var count = $("#reg_relation li").size();
                    if(count < 1)
                        $("#reg_relation").html("<p>선택된 자료가 없습니다.</p>");
                });
            });
            </script>
        </section>
