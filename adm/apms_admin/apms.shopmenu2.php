...
if($mode == 'menu') {
	...
	//Move
	goto_url('./apms.shopmenu2.php?cid='.$cid);
}
...
$sql  = " select * from {$g5['g5_shop_category_table']} where length(ca_id) = '6' and ca_id like '$cid%' order by ca_order asc, ca_id asc ";
...
<style>
	#menu-wrapper { background:#fff; padding:10px 20px; min-width:1800px;}
	.tbl_head01 tbody td.td-inline .frm_input,
	.tbl_head01 tbody td.td-inline select { width: auto; }
</style>

<div id="menu-wrapper">

	<div class="local_desc01 local_desc">
		<b>● "<?php echo $ca['ca_name'];?>"</b> 서브메뉴 설정
	</div>
	...
	<div class="tbl_head01 tbl_wrap">
		...
		<thead>
		<tr>
		<th width=50><nobr>보임</nobr></th>
		<th width=60><nobr>순서</nobr></th>
	...
	<div class="btn_confirm01 btn_confirm" style="text-align:center;">
		<button type="button" class="chk_all btn btn_02">모두보임</button>
		<a href="<?php echo G5_BBS_URL;?>/ficon.php" class="btn btn_02 win_memo">아이콘 검색</a>
		<input type="submit" value="일괄저장" class="btn_submit btn">
		<button type="button" onclick="self.close();" class="btn btn_02">창닫기</button>
	</div>

	</form>
</div>
<script>
	var win_w = screen.width - 100;
	var win_h = screen.height - 140;
	window.moveTo(50, 50);
