...
$row = sql_fetch(" select count(*) as cnt from {$g5['apms_page']} where as_html = '1' ");
$total_count = $row['cnt'];
...
$result = sql_query(" select * from {$g5['apms_page']} where as_html = '1' order by gr_id desc, id limit {$from_record}, {$rows} ");

?>

<style>
.tbl_head01 tbody td.td-inline .frm_input,
.tbl_head01 tbody td.td-inline select,
.tbl_head01.tbl_inline tbody td .frm_input,
.tbl_head01.tbl_inline tbody td select { width: auto; }
.tbl_head01.tbl_inline tbody td { text-align:left; }
</style>

<div class="local_desc01 local_desc">
	<b>문서등록</b> - 사용할 문서파일은 <b>/root/page/</b> 폴더에 올리고, 문서파일 위치는 <b>/root/page/</b>를 기준으로 입력해 주세요.
</div>
...
<div class="tbl_head01 tbl_wrap">
	...
	<td class="td_mng td_mng_m">
		<input type="submit" value="등록" class="btn btn_03">
	</td>
	</tr>
	</tbody>
	</table>
...
<div class="local_desc01 local_desc">
	HTML ID($hid)값으로 체크를 하며, 내용관리 ID 입력시 내용관리에서 해당 ID로 등록한 컨텐츠가 출력되며, 내용관리 > 편집(직접등록) > 문서파일(/page) 순으로 출력됩니다.
	<br>
	메뉴등록시 선택한 보드그룹의 서브메뉴로 출력되며, 등록 후 메뉴설정 > 그룹/분류 > 서브메뉴설정에서 메뉴보임을 체크하셔야 화면에 출력됩니다.
</div>
<div class="tbl_head01 tbl_wrap">
	<table>
	<thead>
	...
	?>
		...
		<td align="center" class="td-inline">
			<nobr>
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, "{$_SERVER['PHP_SELF']}?ap=npage&amp;page="); ?>

<div class="btn_fixed_top">
	<input type="submit" value="선택삭제" onclick="document.pressed=this.value" class="btn btn_02">
	<a href="<?php echo G5_BBS_URL;?>/ficon.php" class="btn btn_02 win_memo">아이콘 검색</a>
	<input type="submit" value="일괄저장" onclick="document.pressed=this.value" class="btn_submit btn">
</div>
