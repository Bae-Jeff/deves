...
$row = sql_fetch(" select count(*) as cnt from {$g5['apms_page']} where as_html = '0' ");
$total_count = $row['cnt'];
...
$result = sql_query(" select * from {$g5['apms_page']} where as_html = '0' order by gr_id desc, id limit {$from_record}, {$rows} ");
...
<div class="local_desc01 local_desc">
	<b>문서등록</b> - 실행파일 위치는 그누보드 루트를 기준으로 실행파일의 경로와 파일명을 입력하시면 됩니다.
</div>
...
<div class="local_desc01 local_desc">
	PAGE ID($pid)값으로 체크를 하며, 실행파일의 위치는 그누보드 루트를 기준으로 실행파일의 경로와 파일명을 입력하시면 됩니다.
	<br>
...
<?php echo get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, "{$_SERVER['PHP_SELF']}?ap=bpage&amp;page="); ?>
