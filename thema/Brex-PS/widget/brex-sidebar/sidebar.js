...
function sidebar_mask(opt) {
	...
	if(opt == 'show') {
		...
		$('body').css({'margin-left': '280px'}, 300);
		$('.under-nav').css({'bottom': '-70px'}, 300);
		$('body').css({'width': body_w+'px'});
	} else {
		//mask.hide();
		$('html, body').css({'overflow': '', 'height': ''});
		$('body').css({'margin-left': '0px'}, 300);
		$('.under-nav').css({'bottom': '0px'}, 300);
		$('body').css({'width': '100%'});
	}
}
...
if(g5_is_member && sidebar_time > 0) {
	setInterval(function() {
		sidebar_response();
		$('#sidebar-response-list').load(sidebar_url + '/response.php');
