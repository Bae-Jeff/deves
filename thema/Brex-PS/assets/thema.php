...
$page_list = array(
	...
	"pid"		=> "pmy,login,reg,regform,regresult,regmail,confirm,password,cart,orderform,orderview,inquiry,inquiryview,ppay,ppayform,ppayresult,partner",
	"hid"		=> "일반문서아이디,일반문서아이디"
);
...
// 기본값 설정
$is_thema_size = ($at_set['size']) ? $at_set['size'] : 980;
$is_page_size = ($at_set['page_size']) ? $at_set['page_size'] : 970;
$is_s_size = ($at_set['s_size']) ? $at_set['s_size'] : 1060;
$is_shop_size = ($at_set['shop_size']) ? $at_set['shop_size'] : 1200;
$is_thema_font = ($at_set['font']) ? $at_set['font'] : 'ko';
if(!$at_set['layout']) $at_set['layout'] = 'wide';
if($is_search || $gid || $bo_table) {
	$is_thema_layout = '';
} else {
...
$is_top_nav = ($at_set['nav']) ? $at_set['nav'] : 'float';
...
if($is_wide_layout) { //메인은 와이드 고정
	...
	if($gid || $is_search) {
		...
		$col_content = ($at_set['page']) ? $at_set['page'] : 8;
	}
}
...
// 좌측형 메뉴간격
$is_menup = (isset($at_set['ms']) && $at_set['ms'] > 0) ? $at_set['ms'] : 15;

// 메뉴높이
$is_smenuh = (isset($at_set['smh']) && $at_set['smh'] > 0) ? $at_set['smh'] : 52;

// 좌측형 메뉴간격
$is_smenup = (isset($at_set['sms']) && $at_set['sms'] > 0) ? $at_set['sms'] : 15;
...
//에드온
if(!$at_set['add']) $at_set['add'] = "a1";
if(!$at_set['sadd']) $at_set['sadd'] = "b1";
?>
<style>
	<?php if($page_background) { ?>	
	body { <?php echo $page_background;?> }
	<?php } ?>
	<?php if($is_index && !IS_SHOP) {?>
	...
	.at-container {max-width:<?php echo $is_s_size;?>px;}
	.no-responsive .wrapper, .no-responsive .at-container-wide { min-width:<?php echo $is_s_size;?>px; }
	.no-responsive .boxed.wrapper, .no-responsive .at-container { width:<?php echo $is_s_size;?>px; }
	<?php } else if(IS_SHOP) {?>
	.at-container {max-width:<?php echo $is_shop_size;?>px;}
	.no-responsive .wrapper, .no-responsive .at-container-wide { min-width:<?php echo $is_shop_size;?>px; }
	.no-responsive .boxed.wrapper, .no-responsive .at-container { width:<?php echo $is_shop_size;?>px; }
	<?php } else {?>
	.at-container {max-width:<?php echo $is_page_size;?>px;}
	.no-responsive .wrapper, .no-responsive .at-container-wide { min-width:<?php echo $is_page_size;?>px; }
	.no-responsive .boxed.wrapper, .no-responsive .at-container { width:<?php echo $is_page_size;?>px; }
	<?php } ?>
	<?php if($is_search) {?>
	...
	.sub-menu, .sub-menu .nav-full-back, .sub-menu .nav-full-height { height:<?php echo $is_smenuh;?>px; }
	.sub-menu .nav-top.nav-float .menu-li { padding:0px <?php echo $is_smenup;?>px; }
	.sub-menu .nav-top.nav-float .sub-1div::before { left: <?php echo $is_smenup;?>px; }
	.sub-menu .subm-w { width:<?php echo $is_subw;?>px; }
	.s-menu, .s-menu .nav-full-back, .s-menu .nav-full-height { height:52px; }
	.s-menu .nav-top.nav-float .menu-li { padding:0px 15px; }
	.s-menu .nav-top.nav-float .sub-1div::before { left: <?php echo $is_smenup;?>px; }
	.s-menu .subm-w { width:<?php echo $is_subw;?>px; }
	.shop-menu, .shop-menu .nav-full-back, .shop-menu .nav-full-height { height:42px; }
	.shop-menu .nav-top.nav-float .menu-li { padding:0px  <?php echo $is_smenup;?>px; }
	.shop-menu .nav-top.nav-float .sub-1div::before { left: <?php echo $is_smenup;?>px; }
	.shop-menu .subm-w { width:<?php echo $is_subw;?>px; }
	<?php if($is_index) {?>
	...
	<?php } else if($is_search) {?>
	@media all and (min-width:1200px) {
		.responsive .boxed.wrapper { max-width:<?php echo $is_s_size;?>px; }
	}
	<?php } else {?>
	@media all and (min-width:1200px) {
		.responsive .boxed.wrapper { max-width:<?php echo $is_page_size;?>px; }
	}
	<?php } ?>
</style>
